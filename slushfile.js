...
var defaults = (function () {
	...
	if (process.platform === 'win32') {
		...
		osUserName = process.env.USERNAME || path
			.basename(homeDir)
			.toLowerCase();
	} else {
		homeDir = process.env.HOME || process.env.HOMEPATH;
		osUserName = homeDir && homeDir
			.split('/')
			.pop() || 'root';
	}
...
gulp.task('default', function (done) {
	...
	inquirer.prompt(prompts, function (answers) {
		...
		gulp
			.src(__dirname + '/templates/' + answers.template + '/**')
		...
		gulp
			.src(__dirname + '/templates/' + answers.template + '/*.zip')
			.pipe(unzip())
			.pipe(gulp.dest('./'));
